...
print OUTFILE5P "==================================================================================\n";
print OUTFILE5P "The Count5and3endAroundStop_v1.pl was run at $timestamp \n";
...
print OUTFILE3P "==================================================================================\n";
print OUTFILE3P "The Count5and3endAroundStop_v1.pl was run at $timestamp \n";
...
#------------------------------------------------------------------------------
$minlen = $lpos+3; #For detail, see the OBJECTIVE section above
...
#9	mRNA	-	ID=GRMZM2G354611_T01;Parent=GRMZM2G354611;Name=GRMZM2G354611_T01;biotype=protein_coding	5	68562,68582;67887,68432;67067,67141;66607,66670;66347,66534
		...
		if ($strandinfo[0] eq '+') { #forward. unfortunately use the first CDS strand info only
			...
			my @lastCDS = split(/\,/, $CDSinfo[$numCDS-1]); #get last position of CDS ( = +2 position )
			#write out the positive side position from the last position
			# eg. position 3 to 50
			for (my $i = $lastCDS[1]+1; $i < $lastCDS[1]+$rpos-1; $i++) { #neg region
				push (@plist, $i);
			}
			
			#write out STOP position
			unshift (@plist,($lastCDS[1]-2,$lastCDS[1]-1,$lastCDS[1]));
			
			#Write out the negative side position, tricky from differ exons
			my $neednt = $lpos;
			my $added = 0; #count how many unshifted position
			my $exon = $numCDS-1; #last exon
			my @pospair = split (/\,/,$CDSinfo[$exon]);
			my $current = $pospair[1]-3; #three nt backward (STOP codon already done)
			while ($added < $neednt){
				if ($current >= $pospair[0]) { #within exon
					unshift (@plist, $current);
					$added++; #count
					$current--; #move next
				} else { #outside exon
					$exon--; #move to next exon
					if ($exon < 0) { print "!!!Negative exon call\n"; }
			...
			#write out the positive side position from the last position
			# eg. position 3 to 50
			for (my $i = $lastCDS[0]-1; $i > $lastCDS[0]-$rpos+1; $i--) { #neg region
				push (@plist, $i);
			}
			
			#write out STOP position
			unshift (@plist,($lastCDS[1]+2,$lastCDS[1]+1,$lastCDS[0]));
			...
			my $exon = $numCDS-1; #last exon
			my @pospair = split (/\,/,$CDSinfo[$exon]);
			my $current = $pospair[0]+3; #three nt backward (STOP codon already done)
			while ($added < $neednt){
